...
def test_full_translate_questions_job_lifecycle():
    create_response = client.post(
    ...
    job = create_response.json()
    assert job["status"] == "analyze_pending"
    assert job["job_kind"] == "question_translation"
    assert job["interview_language"] == "ms"
    job_id = job["id"]
    ...
    claim = claim_response.json()
    assert claim["job_id"] == job_id
    assert claim["phase"] == "translate_questions"
    assert claim["interview_language"] == "ms"
    assert len(claim["analysis_questions"]) == 1

    complete_response = client.post(
        f"/v1/worker/jobs/{job_id}/complete-question-translation",
        headers=WORKER_HEADERS,
        json={
            "result": {
                "questions": [
                    {
                        "id": "device-type",
                        "prompt_conduct": "Apakah jenis peranti mobiliti itu?",
                        "hint_conduct": "cth. PMD",
                        "section_conduct": "Peranti terjejas",
                    }
                ],
                "source": "fake",
            }
    ...
    completed = complete_response.json()
    assert completed["status"] == "completed"
    assert completed["analysis_result"]["questions"][0]["prompt_conduct"]
